...
class ProfilePage(QWidget):
    def __init__(self, person):
        ...
        form.addRow("Passenger Rating:", QLabel(str(self.person.passenger_rating)))
        self.driverLabel = QLabel(str(self.person.driver_rating))
        self.driverLabelText = QLabel("Driver Rating:")
        form.addRow(self.driverLabelText, self.driverLabel)
        if person.is_driver == 0:
            self.driverLabel.hide()
            self.driverLabelText.hide()
    ...
    def driver_toggle(self):
        if self.driver_checkbox.isChecked():
            ...
            self.driverLabel.show()
            self.driverLabelText.show()
        else:
            self.driver_dashboard_button.hide()
            self.pending_requests_button.hide()
            self.driverLabel.hide()
