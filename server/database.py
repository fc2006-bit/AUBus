...
def login_user(username: str, password: str) -> str:
    ...
    with sqlite3.connect(DB_FILE) as conn:
        ...
        return f"success:{name}:{email}:{area}:{is_driver}:{min_passenger_rating}:{driver_rating}:{passenger_rating}:{pending_requests}:{mon_commute}:{tue_commute}:{wed_commute}:{thu_commute}:{fri_commute}:{sat_commute}:{sun_commute}"
